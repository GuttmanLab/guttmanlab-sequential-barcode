...
public class BasicBarcodedFragment implements BarcodedFragment {
	 ...
	 */
	private BarcodeSequence findAndAppendBarcodes(ReadLayout readLayout, List<List<ReadSequenceElement>> readElements) {
			...
			for(int i = 0; i < readElements.size(); i++) {
				...
				if(cl.equals(FragmentBarcode.class) || cl.equals(BarcodeSet.class)) {
					for(ReadSequenceElement elt : readElements.get(i)) {
						barcodeList.add(((FragmentBarcode)elt).getBarcode());
					}
					continue;
				}
				if(cl.equals(BarcodeEquivalenceClass.class) || cl.equals(BarcodeEquivalenceClassSet.class)) {
					for(ReadSequenceElement elt : readElements.get(i)) {
						...
						barcodeList.add(bec.toBarcode().getBarcode());
					}
					continue;
				}
			}
		}
		return new BarcodeSequence(barcodeList);
	}
	 ...
	 * @param matchedEltsRead2 Matched elements for read 2 or null if identifying here for the first time or there is no read 2
	 */
	private final BarcodeSequence findBarcodes(List<List<ReadSequenceElement>> matchedEltsRead1, List<List<ReadSequenceElement>> matchedEltsRead2) {
		if(read1layout != null && read1sequence != null) {
			List<List<ReadSequenceElement>> read1elements = 
					...
					if(read1elements != null) {
						return findAndAppendBarcodes(read1layout, read1elements);
					} else throw new IllegalStateException("Read 1 elements is null");
		}
		if(read2layout != null && read2sequence != null) {
			List<List<ReadSequenceElement>> read2elements = 
					...
					if(read2elements != null) {
						return findAndAppendBarcodes(read2layout, read2elements);
					} else throw new IllegalStateException("Read 2 elements is null");
		}
		throw new IllegalStateException("Read layout or sequence is null for both reads");
	}
